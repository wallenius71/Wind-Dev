                ...
                else
                {
                    ...
                    List<double> shear = new List<double>(_filtereddata.Count);
                        ...
                        //use upper ws for counts and frequencies
                            ...
                            upperWS = thisRowsValues.Where(c => c.UpperWS >= start & c.UpperWS < end).Select(c => c.UpperWS).ToList();
                            lowerWS = thisRowsValues.Where(c => c.LowerWS >= start & c.LowerWS < end).Select(c => c.LowerWS).ToList();
                            shear = thisRowsValues.Where(c => c.Shear >= start & c.Shear < end).Select(c => c.Shear).ToList();
