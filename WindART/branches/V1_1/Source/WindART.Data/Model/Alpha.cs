...
namespace WindART
{
    public class Alpha:AbstractAlpha 
    {
        ...
        public override void CalculateAlpha()
        {
            ...
            {
                if (_alphaFilter == AlphaFilterMethod.NonCoincident)
                {
                    _coincident = _data.AsEnumerable().ToList();
                }
            }
            ...
            //the defualt sessioncolindex for  ws axis is upper comp, 
            ...
            //row filter is the class that will filter re: the axes selected
           ...
           //fill alpha table
            ...
            for (int a = 0; a < x; a++)
            {
                for (int j = 0; j < y; j++)
                {
                    ...
                    //build up numerator bulk calculation
                    numerator += Alpha[a, j];

                }
            }
            denominator=x*y;
            AllAlphaAvg = numerator / denominator;
